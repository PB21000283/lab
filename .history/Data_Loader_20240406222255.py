...
def TrainValueDataLoader(train_file, poi_file, batchsize):
    def collate_fn_neg(data):##用于在数据加载过程中对数据批次进行特定的预处理。
        ...
        traj_poi_pos_tensor=pad_to_length( traj_poi_pos_tensor,args.seq_len, padding_value=0)
        traj_poi_neg_tensor=pad_to_length(traj_poi_neg_tensor.seq_len, padding_value=0)
        poi_pos_tensor=pad_to_length(poi_pos_tensor,args.seq_len, padding_value=0)
        poi_neg_tensor=pad_to_length(poi_neg_tensor.seq_len, padding_value=0)
