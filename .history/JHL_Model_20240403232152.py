...
class GRLSTM(nn.Module):
    ...
    ##第一行包含起点的索引。
    ##第二行包含终点的索引。
    def forward(self, data_forgat, data_forgat_len,data_input ,padding_masks,output_hidden_states=False, output_attentions=False,
                pos=True):
        if pos :
            batch_x, batch_x_len = data_forgat ,data_forgat_len #atch_x可能具有形状(batch_size, seq_len)，这里的batch_size是批次中轨迹的数量，seq_len是轨迹长度。
            ...
            ##TODO
            padding_masks_input = padding_masks.unsqueeze(1).repeat(1, batch_x_len, 1).unsqueeze(1)  # (B, 1, T, T)
        ...
        return  batch_emb_pack, all_hidden_states, all_self_attentions , traj_poi_emb,  poi_emb # (B, T, d_model), list of (B, T, d_model), list of (B, head, T, T)
            ##TODO
...
class JHL_Model(nn.Module):
    ...
    def forward(self, data_forgat,data_forgat_len,data_input,##TODO，argument_methods==false,代表不再进行数据增强
        ...
        if type ==True :##一个是处理数据，一个是掩码
            out_view1, hidden_states1, traj_poi_emb,  poi_emb = self.grlstm(data_forgat,data_forgat_len, data_input,padding_masks=padding_masks, ##把bert改了？
            ...
            mask_output, _, traj_poi_emb,  poi_emb = self.grlstm(data_forgat,data_forgat_len, data_input,padding_masks=padding_masks,
                                    output_hidden_states=output_hidden_states,
                                    output_attentions=output_attentions, pos=pos)  # (B, T, d_model)
            a = self.mask_l(mask_output)
