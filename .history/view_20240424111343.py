...
# Get the length of each element in train_list
lengths = np.array([len(x) for x in data['train_list']])

# Get the indices that would sort the array by length
sorted_indices = np.argsort(lengths)

# Sort both train_list and train_y using these indices
data['train_list'] = data['train_list'][sorted_indices]
data['train_y'] = data['train_y'][sorted_indices]

print("Sorted train_list:", data['train_list'])
print("Corresponding train_y:", data['train_y'])

x=[torch.LongTensor(sq) for sq in data['train_list']]
y=[sq for sq in data['train_y']]
print(x[1])
print(y[1])
