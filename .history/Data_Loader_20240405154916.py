...
class MyData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return tuple_ , mask##TODO，注意mask的格式
...
def TrainValueDataLoader(train_file, poi_file, batchsize):
    def collate_fn_neg(data,max_len,vocab,add_cls):##用于在数据加载过程中对数据批次进行特定的预处理。
        ...
        padding_mask_x=padding_mask(torch.tensor(batch_x_lengths, dtype=torch.int16), max_batch_x_lengths=max( batch_x_lengths))

        padding_mask_y=padding_mask(torch.tensor(batch_y_lengths, dtype=torch.int16), max_batch_y_lengths=max( batch_y_lengths))

        padding_mask_n=padding_mask(torch.tensor(batch_n_lengths, dtype=torch.int16), max_batch_n_lengths=max( batch_n_lengths))
        ...
        return data, data_neg, data_label, data_length, neg_length, label_length, \
               ...
               masked_x, targets, target_masks, padding_masks,padding_mask_x,  padding_mask_y, padding_mask_n
