...
def collate_unsuperv_mask(data, max_len=None, vocab=None, add_cls=True):
    ...
    X[..., 0:1].masked_fill_(target_masks[..., 0:1] == 1, 3)  # loc -> mask_index
...
def TrainValueDataLoader(train_file, poi_file, batchsize):
    def collate_fn_neg(data,max_len,vocab,add_cls):##用于在数据加载过程中对数据批次进行特定的预处理。
        ...
        padding_masks_x=padding_mask(torch.tensor(batch_x_lengths, dtype=torch.int16), max_batch_x_lengths=max( batch_x_lengths))

        padding_masks_y=padding_mask(torch.tensor(batch_y_lengths, dtype=torch.int16), max_batch_y_lengths=max( batch_y_lengths))

        padding_masks_n=padding_mask(torch.tensor(batch_n_lengths, dtype=torch.int16), max_batch_n_lengths=max( batch_n_lengths))
