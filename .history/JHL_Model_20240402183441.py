...
class GRLSTM(nn.Module):
    ...
    ##第一行包含起点的索引。
    ##第二行包含终点的索引。
    def forward(self, data, data_len, padding_masks,output_hidden_states=False, output_attentions=False,
                pos=True):
        if pos :
            batch_x, batch_x_len, _, _ = data ,data_len #atch_x可能具有形状(batch_size, seq_len)，这里的batch_size是批次中轨迹的数量，seq_len是轨迹长度。
...
class JHL_Model(nn.Module):
    ...
    def forward(self, data,data_len,##TODO，argument_methods==false,代表不再进行数据增强
                padding_masks,
        ...
        if type ==True :##一个是处理数据，一个是掩码
            out_view1, hidden_states1, _ = self.grlstm(data,data_len, padding_masks=padding_masks, ##把bert改了？
            ...
            bert_output, _, _ = self.grlstm(data,data_len, padding_masks=padding_masks,
                                    output_hidden_states=output_hidden_states,
                                    output_attentions=output_attentions)  # (B, T, d_model)
