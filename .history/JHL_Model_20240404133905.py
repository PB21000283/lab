...
class JHL_Model(nn.Module):
    ...
    def forward(self, data_forgat,data_forgat_len, poi, batch_traj_poi,data_input,##TODO，argument_methods==false,代表不再进行数据增强
        ...
        if type ==True :##一个是处理数据，一个是掩码
            out_view1, hidden_states1, traj_poi_emb,  poi_emb = self.grlstm((data_forgat,data_forgat_len ,poi, batch_traj_poi), data_input,padding_masks=padding_masks, ##把bert改了？
            ...
            mask_output, _, traj_poi_emb,  poi_emb = self.grlstm((data_forgat,data_forgat_len, poi, batch_traj_poi), data_input,padding_masks=padding_masks,
                                    output_hidden_states=output_hidden_states,
                                    output_attentions=output_attentions, pos=pos)  # (B, T, d_model)
