...
mask[0]= torch.LongTensor(noise_mask(data_idx_transformed, 0.2, 3, "together", "random", None, True))

data_idx_array = np.array(y[1])

data_idx_transformed= np.array([[item] for item in data_idx_array])

mask[1]= torch.LongTensor(noise_mask(data_idx_transformed, 0.2, 3, "together", "random", None, True))
...
data_for_mask = list(zip(features,mask))
