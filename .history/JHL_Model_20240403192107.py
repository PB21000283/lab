...
class GRLSTM(nn.Module):
    ...
    ##第一行包含起点的索引。
    ##第二行包含终点的索引。
    def forward(self, data, data_len,data_input ,padding_masks,output_hidden_states=False, output_attentions=False,
                pos=True):
        if pos :
            batch_x, batch_x_len = data ,data_len #atch_x可能具有形状(batch_size, seq_len)，这里的batch_size是批次中轨迹的数量，seq_len是轨迹长度。
