...
class Trainer:
    ...
    def _pass(self, data, train=True):
        ...
        ##TODO，batch_x的格式是？
        last_output_emb, poi_emb, traj_poi_emb = self.model(batch_x, batch_x_len,poi_pos,batch_traj_poi_pos ,batch_x ,
        ...
        ##TODO,loss4 掩码恢复损失
        loss = loss1  + mean_loss_l +loss2+loss3
...
##TODO!!!,只取一个批次
    ...
    def train(self):   
        for epoch in range(self.n_epochs):##n_epochs周期数
            ...
            test_loss = self._val_epoch()
            logging.info(
                '[Epoch %d/%d] [training loss: %f] [testing loss: %f]' %
                (epoch, self.n_epochs, train_loss, test_loss)
            )
