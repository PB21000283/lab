...
def collate_unsuperv_mask( tra_len, features,masks, max_len=None, vocab=None, add_cls=True):
    ...
    ##TODO features是一个list,每个list含seq_length个点，每个点有feat_dim个特征
    ...
    padding_masks = padding_mask(torch.tensor(lengths, dtype=torch.int16), max_len=max_len)##有效长度掩码
    ##取反后，True表示模型需要关注的有效数据位置
...
mask_x = noise_mask(np.array(x[1]), 0.2, 3, "together", "random", None, True) 
mask_x_2= np.array([[item] for item in mask_x])
print(mask_x_2 )
