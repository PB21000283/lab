...
def TrainValueDataLoader(train_file, poi_file, batchsize):
    def collate_fn_neg(data,max_len,vocab,add_cls):##用于在数据加载过程中对数据批次进行特定的预处理。
        ...
        ##TODO!,这里有问题，features需要转换格式才有shape[0]
        features=[(sq[0]) for sq in data_tuple]
        #和上面的mydata对应
        ##data的格式？
        ...
        return data, data_neg, data_label, data_length, neg_length, label_length, \
               traj_poi_pos_tensor, traj_poi_neg_tensor, poi_pos_tensor, poi_neg_tensor,masked_x, targets, target_masks, padding_masks##TODO，mask       
