...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, d_model, attn_heads, feed_forward_hidden, drop_path,
        ...
        self.drop_path = D+ropPath(drop_path) if drop_path > 0. else nn.Identity()
...
class GRLSTM(nn.Module):
    ...
    ##第一行包含起点的索引。
    ##第二行包含终点的索引。
    def forward(self, fps, padding_masks,output_hidden_states=False, output_attentions=False,
                pos=True):
        if pos :
...
class JHL_Model(nn.Module):
    ...
    def forward(self, data,##TODO，argument_methods==false,代表不再进行数据增强
                padding_masks,
        ...
        if type ==True :##一个是处理数据，一个是掩码
            out_view1, hidden_states1, _ = self.grlstm(x=data, padding_masks=padding_masks, ##把bert改了？
            ...
            bert_output, _, _ = self.grlstm(x=data, padding_masks=padding_masks,
                                    output_hidden_states=output_hidden_states,
                                    output_attentions=output_attentions)  # (B, T, d_model)
