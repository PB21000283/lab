...
def TrainValueDataLoader(train_file, poi_file, batchsize):
    def collate_fn_neg(data_tuple,max_len,vocab,add_cls):##用于在数据加载过程中对数据批次进行特定的预处理。
    ...
    dataset = DataLoader(data_, batch_size=batchsize, shuffle=True, 
                        collate_fn=lambda x: collate_fn_neg(x, max_len=self.seq_len,
