...
def collate_unsuperv_mask(data, max_len=None,  add_cls=True):
    ...
    for i in range(batch_size):
        ...
        X[i, :end] = features[i][:end]
        target_masks[i, :end] = masks[i][:end].squeeze()
        ##print(masks[i][:end].squeeze())
    padding_masks = padding_mask(torch.tensor(lengths, dtype=torch.int16), max_len=max_len)
    target_masks = ~target_masks  # (batch_size, padded_length, feat_dim)
    target_masks = target_masks * padding_masks
    targets = X.clone()
    targets = targets.masked_fill_(target_masks == 0,0)
    ##print(masks[0][:end].squeeze())
    ##print( targets[0])
...
def pad_to_length(data, pad_length, padding_value=0):
    """
    将一个序列列表填充（或截断）到指定的长度。
    
    Args:
    - data (list of Tensor): 要填充的序列列表。
    - pad_length (int): 指定的长度。
    - padding_value (int, optional): 用于填充的值，默认为 0。
    
    Returns:
    - Tensor: 填充（或截断）后的序列张量。
    """
    # 将序列填充到最大长度
    padded_data = rnn_utils.pad_sequence(data, batch_first=True, padding_value=padding_value)
    
    # 如果指定的长度小于最大长度，则截断；如果大于，则填充
    if pad_length < padded_data.shape[1]:
        return padded_data[:, :pad_length]
    else:
        # 需要额外填充的长度
        extra_pad = pad_length - padded_data.shape[1]
        # 在序列后填充额外的padding_value
        extra_padding = torch.full((padded_data.shape[0], extra_pad, *padded_data.shape[2:]), padding_value, dtype=padded_data.dtype)
        return torch.cat([padded_data, extra_padding], dim=1)
##TODO!!,有个问题，所有批次的轨迹都固定长度？还是每个批次的轨迹长度一样就行？
...
def TrainValueDataLoader(train_file, poi_file, batchsize):
    def collate_fn_neg(data):##用于在数据加载过程中对数据批次进行特定的预处理。
        ...
        ##TODO!!这里先把降序排布去掉,之后要补上！！！！传给注意力网络的数据要降序排布
        ...
        padding_masks_x=padding_mask(torch.tensor(batch_x_lengths, dtype=torch.int16), max_len=args.seq_len)

        ##print(max( batch_x_lengths))

        padding_masks_y=padding_mask(torch.tensor(batch_y_lengths, dtype=torch.int16), max_len=args.seq_len)

        padding_masks_n=padding_mask(torch.tensor(batch_n_lengths, dtype=torch.int16), max_len=args.seq_len)

        ##TODO!!!这里是用0填充
        data =pad_to_length(data,args.seq_len, padding_value=0)
        data_label=pad_to_length( data_label,args.seq_len, padding_value=0)
        data_neg =pad_to_length(data_neg,args.seq_len, padding_value=0)
        masked_x=pad_to_length(masked_x,args.seq_len, padding_value=0)
        traj_poi_pos_tensor=pad_to_length( traj_poi_pos,args.seq_len, padding_value=0)
        traj_poi_neg_tensor=pad_to_length(traj_poi_neg,args.seq_len, padding_value=0)
        poi_pos_tensor=pad_to_length(poi_pos,args.seq_len, padding_value=0)
        poi_neg_tensor=pad_to_length(poi_neg,args.seq_len, padding_value=0)

        ##data = rnn_utils.pad_sequence(data, batch_first=True, padding_value=0)
        traj_poi_pos_tensor = rnn_utils.pad_sequence(traj_poi_pos_tensor, batch_first=True, padding_value=0)
        traj_poi_neg_tensor = rnn_utils.pad_sequence(traj_poi_neg_tensor, batch_first=True, padding_value=0)
        ...
        poi_pos_tensor = rnn_utils.pad_sequence(poi_pos_tensor, batch_first=True, padding_value=0)
        poi_neg_tensor = rnn_utils.pad_sequence(poi_neg_tensor, batch_first=True, padding_value=0)
...
def TrainDataValLoader(train_file, batchsize):
    def collate_fn_neg(data):
        ...
        padding_masks_x=padding_mask(torch.tensor(batch_x_lengths, dtype=torch.int16), max_len=args.seq_len)
        padding_masks_x=rnn_utils.pad_sequence(padding_masks_x, batch_first=True, padding_value=0)
        return data, label, data_length, idx_list , padding_masks_x
