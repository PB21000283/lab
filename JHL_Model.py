...
class GRLSTM(nn.Module):
    ...
    ##第一行包含起点的索引。
    ##第二行包含终点的索引。
    def _sample_span(self, x, padding_masks, sample_rate=0.2):##对输入数据张量 x 进行区间采样，以增强数据的多样性。
    ...
    def _cutoff_embeddings(self, embedding_output, padding_masks, direction, cutoff_rate=0.2):##用于对嵌入向量进行截断，以增加数据的稀疏性或鲁棒性。
    ...
    def _shuffle_embeddings(self, embedding_output, padding_masks):##用于对嵌入向量进行随机重排，以增加模型的鲁棒性。
        batch_size, seq_len, d_model = embedding_output.shape
        shuffled_embeddings = []
        for bsz_id in range(batch_size):
